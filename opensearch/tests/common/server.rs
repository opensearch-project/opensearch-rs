...
impl MockServerBuilder {
    ...
    pub async fn start(self) -> anyhow::Result<MockServer> {
        ...
        thread::spawn(move || {
            ...
            let response_delay = self.response_delay.clone();
                    ...
                    tokio::task::spawn(async move {
                        let conn = http1::Builder::new().serve_connection(
                            ...
                            service_fn(move |req| {
                                let requests_tx = requests_tx.clone();
                                async move {
                                    requests_tx.send(req.into())?;
                                    if let Some(response_delay) = response_delay.clone() {
                                        tokio::time::sleep(response_delay).await;
                                    }
                                    Ok::<_, anyhow::Error>(Response::<Empty<Bytes>>::default())
                                }
                            }),
                        );
                        tokio::pin!(conn);
        ...
        .join()
        .map_err(|e| anyhow::anyhow!("MockServer construction failed: {:?}", e))
    }
}
...
impl MockServer {
    ...
    pub async fn start() -> anyhow::Result<MockServer> {
