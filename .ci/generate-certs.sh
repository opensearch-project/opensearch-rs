...
esnode_no_san_crt="$certs_dir/esnode-no-san.crt"
...
if [[ ! -f $esnode_csr ]]; then
    rm -f $esnode_crt $esnode_no_san_crt
    openssl req -new \
        $common_csr_args \
        -key $esnode_key \
...
if [[ ! -f $esnode_no_san_crt ]]; then
    openssl x509 -req -in $esnode_csr $common_crt_args -extensions esnode-no-san -out $esnode_no_san_crt
fi
